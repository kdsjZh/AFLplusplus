...
def parse_fid_log(fid_name):
  ...
  with open(fid_name) as f:
    for line in f:
      fname, id_str, start_bb, end_bb = line.strip('\n').split(',')
...
def match_node_with_fid(base_dir, module_name):
  node2fname = parse_cg_log('%s/%s%s' % (base_dir, module_name, __prefix_name['cg']))
  fname2id = parse_fid_log('%s/fid' % (base_dir))
  node2id = dict()
...
def main():
  ...
  parser.add_argument ('-i', type=str, required=True, help="Path to base directory containing cg and fid")
  parser.add_argument ('-m', type=str, required=True, help="Module name")
  args = parser.parse_args()
  # build a cg node to function id vector
  module2node = dict()
  # only one module file in LTO mode
  print ("[*] Working with %s..." % (args.m))
  node2id = match_node_with_fid(args.i, args.m)
  dump_node_id(args.i, args.m, node2id)
  # done dumping node id

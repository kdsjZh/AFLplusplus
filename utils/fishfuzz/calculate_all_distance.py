...
def main():
  ...
  # for LTO, only calculate once
  print ("[*] Distance Calculation for %s..." % (args.m))
  cg_name = '%s%s' % (args.m, __prefix_name['cg'])
  node_map_name = '%s%s' % (args.m, __prefix_name['node'])
  cg = nx.DiGraph(nx.nx_pydot.read_dot('%s/%s' % (args.i, cg_name)))
  ...
  dump_module_dist(args.i, args.m, module_calldst)
